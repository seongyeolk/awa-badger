...
class Interface(interface.Interface):
    ...
    def __init__(self):
        ...
        self.cameras = {
            "YAG1": AWACamera("NIFG", self.testing),
            "YAG2": AWACamera("AWAPG", self.testing),
        }

    def get_value(self, channel: str):
        if not self.testing:
            if channel in self.epics_pvs:
                return caget(channel)
        else:
            return 1.0

    def get_camera_measurement(self, camera_name, target_charge, n_samples=1) -> dict:
        return self.cameras[camera_name].get_measurement(
            target_charge, n_samples=n_samples
        )
